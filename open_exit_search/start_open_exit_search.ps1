...
# Runs prep -> stage0 (ablation) -> stage1 -> stage2 -> stage3 (checks) -> stage4 (FTMO 2-step combos) in ONE hidden,
# BelowNormal-priority Python process with a worker pool. Resumable: re-running skips stage-1 work already saved.
#
#   cd C:\Users\Administrator\MeanReversion\open_exit_search
#   .\start_open_exit_search.ps1                 # defaults: 4 workers, 30,000 configs per instrument (~2-3 h)
#   .\start_open_exit_search.ps1 -N1 50000       # bigger stage 1
#
...
$argLine = "-3 oe_search.py all --workers $Workers --n1 $N1 --top2 $Top2 --nn $NN --k3 $K3 --k4 $K4 --top4 $Top4 --inst $Inst"
...
Write-Host "Started open_exit_search (PID $($p.Id)), $Workers workers, N1=$N1 per instrument." -ForegroundColor Green
